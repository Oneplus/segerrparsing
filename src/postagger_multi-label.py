  ...
  with codecs.open(path, 'r', encoding='utf-8') as fin:
    for lines in fin.read().strip().split('\n\n'):
      ...
      for line in lines.splitlines():
        ...
        pos.append([tokens[3], tokens[4]])
      dataset.append(data)
      pos_dataset.append(pos)
  return dataset, pos_dataset
...
def create_one_batch(x, e, y, word2id, char2id, cluster2id, oov='<oov>', pad='<pad>', sort=True, use_cuda=False):
  ...
  batch_y_upos = torch.LongTensor(batch_size, max_len).fill_(0)
  for i, y_i in enumerate(y):
    for j, y_ij in enumerate(y_i):
      batch_y_upos[i][j] = y_ij[0]

  batch_y_xpos = torch.LongTensor(batch_size, max_len).fill_(0)
  for i, y_i in enumerate(y):
    for j, y_ij in enumerate(y_i):
      batch_y_xpos[i][j] = y_ij[1]
  if use_cuda:
    batch_x = batch_x.cuda()
    batch_c = batch_c.cuda()
    batch_y_upos = batch_y_upos.cuda()
    batch_y_xpos = batch_y_xpos.cuda()
    indices = indices.cuda()

  return batch_x, (batch_c, new_batch_c_lens.tolist(), indices), batch_e, batch_cluster, (batch_y_upos, batch_y_xpos), lens
...
class EnsembleModel(nn.Module):
  ...
  def forward(self, word_inp, chars_package, elmo, cluster_inp, y):
    ...
    upos_probs = self.models[0].classify_layer_upos.get_probs(output)
    xpos_probs = self.models[0].classify_layer_xpos.get_probs(output)
    for model in self.models[1:]:
      output = model.encoder_output(word_inp, chars_package, elmo, cluster_inp)
      upos_probs.add_(model.classify_layer_upos.get_probs(output))
      xpos_probs.add_(model.classify_layer_xpos.get_probs(output))
    upos_probs.div_(self.n_models)
    xpos_probs.div_(self.n_models)

    _, upos_result = torch.max(upos_probs[:, :, 1:], 2)
    _, xpos_result = torch.max(xpos_probs[:, :, 1:], 2)

    upos_result.add_(1)
    xpos_result.add_(1)

    return (upos_result, xpos_result), torch.FloatTensor([0.0])

class Model(nn.Module):
  def __init__(self, opt, word_emb_layer, char_emb_layer, cluster_emb_layer, n_upos, n_xpos, use_cuda):
    ...
    if opt.classifier.lower() == 'vanilla':
      ...
      self.classify_layer_upos = classify_layer_func(encoder_output, n_upos, self.use_cuda)
      self.classify_layer_xpos = classify_layer_func(encoder_output, n_xpos, self.use_cuda)
  ...
  def forward(self, word_inp, chars_package, elmo, cluster_inp, y):
    ...
    output_upos, loss_upos = self.classify_layer_upos.forward(output, y[0])
    output_xpos, loss_xpos = self.classify_layer_xpos.forward(output, y[1])

    if not self.training:
      self.classify_time += time.time() - start_time

    loss = loss_upos + loss_xpos

    if self.training:
      loss += self.opt.l2 * self.classify_layer_upos.hidden2tag.weight.data.norm(2)
      loss += self.opt.l2 * self.classify_layer_xpos.hidden2tag.weight.data.norm(2)

    return (output_upos, output_xpos), loss
...
def eval_model(model, valid, ix2upos, ix2xpos, args, gold_path):
  ...
  for x, c, e, cluster, y, lens, text in zip(valid_x, valid_c, valid_e, valid_cluster, valid_y, valid_lens, valid_text):
    ...
    output_data = [[[u, x] for u, x in zip(upos, xpos)] for upos, xpos in zip(output[0].data, output[1].data)]
    for bid in range(len(x)):
      for k, (word, tag) in enumerate(zip(text[bid], output_data[bid])):
        upos = ix2upos[int(tag[0])]
        xpos = ix2xpos[int(tag[1])]
        print('{0}\t{1}\t{1}\t{2}\t{3}\t_\t_\t_'.format(k + 1, word, upos, xpos), file=fpo)
      print(file=fpo)
  fpo.close()
  ...
  f_upos = float(lines[-2].strip().split()[-1])
  f_xpos = float(lines[-1].strip().split()[-1])
  os.remove(path)
  return (f_upos, f_xpos)

def train_model(epoch, model, optimizer,
                train, valid, test, ix2upos, ix2xpos, best_valid, test_result):
  model.train()
  opt = model.opt
  ...
  for x, c, e, cluster, y, lens in zip(train_x, train_c, train_e, train_cluster, train_y, train_lens):
    ...
    if cnt % opt.eval_steps == 0:
      valid_result = eval_model(model, valid, ix2upos, ix2xpos, opt, opt.gold_valid_path)
      logging.info("Epoch={} iter={} lr={:.6f} train_loss={:.6f} valid_upos_acc={:.6f}, valid_xpos_acc={:.6f}".format(
        epoch, cnt, optimizer.param_groups[0]['lr'], total_loss, valid_result[0], valid_result[1]))
      # upos 
      if valid_result[0] > best_valid[0]:
        torch.save(model.state_dict(), os.path.join(opt.model, 'upos_model.pkl'))
        logging.info("New record achieved on UPOS!")
        best_valid[0] = valid_result[0]
        if test is not None:
          test_result[0] = eval_model(model, test, ix2upos, ix2xpos, opt, opt.gold_test_path)[0]
          logging.info("Epoch={} iter={} lr={:.6f} test_upos_acc={:.6f}.".format(
            epoch, cnt, optimizer.param_groups[0]['lr'], test_result[0]))

      # xpos
      if valid_result[1] > best_valid[1]:
        torch.save(model.state_dict(), os.path.join(opt.model, 'xpos_model.pkl'))
        logging.info("New record achieved on XPOS!")
        best_valid[1] = valid_result[1]
        if test is not None:
          test_result[1] = eval_model(model, test, ix2upos, ix2xpos, opt, opt.gold_test_path)[1]
          logging.info("Epoch={} iter={} lr={:.6f} test_xpos_acc={:.6f}.".format(
            epoch, cnt, optimizer.param_groups[0]['lr'], test_result[1]))

  return best_valid, test_result


def label_to_index(y, upos_to_ix, xpos_to_ix, incremental=True):
  for i in range(len(y)):
    for j in range(len(y[i])):
      if y[i][j][0] not in upos_to_ix and incremental:
        upos = upos_to_ix[y[i][j][0]] = len(upos_to_ix)
      else:
        upos = upos_to_ix.get(y[i][j][0], 0)
      y[i][j][0] = upos

      if y[i][j][1] not in xpos_to_ix and incremental:
        xpos = xpos_to_ix[y[i][j][1]] = len(xpos_to_ix)
      else:
        xpos = xpos_to_ix.get(y[i][j][1], 0)
      y[i][j][1] = xpos
...
def train():
  ...
  cmd.add_argument("--cluster_dim", type=int, default=50, help='the cluster dimension.')
  ...
  if opt.use_partial:
      upos_to_ix = {'<pad>': 0, '<UNK>': 1}
      xpos_to_ix = {'<pad>': 0, '<UNK>': 1}
  else:
      upos_to_ix = {'<pad>': 0}
      xpos_to_ix = {'<pad>': 0}

  label_to_index(train_y, upos_to_ix, xpos_to_ix)
  label_to_index(valid_y, upos_to_ix, xpos_to_ix, incremental=False)
  label_to_index(test_y, upos_to_ix, xpos_to_ix, incremental=False)
  logging.info('number of upos: {0}'.format(len(upos_to_ix)))
  logging.info('number of xpos: {0}'.format(len(xpos_to_ix)))
  ...
  n_upos = len(upos_to_ix)
  n_xpos = len(xpos_to_ix)
  ix2upos = {ix: upos for upos, ix in upos_to_ix.items()}
  ix2xpos = {ix: xpos for xpos, ix in xpos_to_ix.items()}
  ...
  model = Model(opt, word_emb_layer, char_emb_layer, cluster_emb_layer, n_upos, n_xpos, use_cuda)
  ...
  with codecs.open(os.path.join(opt.model, 'upos.dic'), 'w', encoding='utf-8') as fpo:
    for upos, i in upos_to_ix.items():
      print('{0}\t{1}'.format(upos, i), file=fpo)  

  with codecs.open(os.path.join(opt.model, 'xpos.dic'), 'w', encoding='utf-8') as fpo:
    for xpos, i in xpos_to_ix.items():
      print('{0}\t{1}'.format(xpos, i), file=fpo)  

  json.dump(vars(opt), codecs.open(os.path.join(opt.model, 'config.json'), 'w', encoding='utf-8'))
  best_valid, test_result = [-1e8, -1e8], [-1e8, -1e8]
  ...
  if opt.use_partial:
      logging.info("best_valid_f: {:.6f}, {:.6f}".format(best_valid[0], best_valid[1]))
      logging.info("test_f: {:.6f}, {:.6f}".format(test_result[0], test_result[1]))
  else:
      logging.info("best_valid_acc: {:.6f}, {:.6f}".format(best_valid[0], best_valid[1]))
      logging.info("test_acc: {:.6f}, {:.6f}".format(test_result[0], test_result[1]))
...
def test():
  ...
  char_lexicon = {}
  upos_char_emb_layers = []
  xpos_char_emb_layers = []
  ...
  for path in models_path:
    upos_char_emb_layers.append(EmbeddingLayer(args2.char_dim, char_lexicon, fix_emb=False, embs=None))
    xpos_char_emb_layers.append(EmbeddingLayer(args2.char_dim, char_lexicon, fix_emb=False, embs=None))


  upos_word_emb_layers = []
  xpos_word_emb_layers = []
  word_lexicon = {}
  ...
  for path in models_path:
    upos_word_emb_layers.append(EmbeddingLayer(args2.word_dim, word_lexicon, fix_emb=False, embs=None))
    xpos_word_emb_layers.append(EmbeddingLayer(args2.word_dim, word_lexicon, fix_emb=False, embs=None))

  logging.info('word embedding size: ' + str(len(upos_word_emb_layers[0].word2id)))

  if args2.use_cluster:
    cluster_lexicon = {}
    upos_cluster_emb_layers = []
    xpos_cluster_emb_layers = []
    ...
    for path in models_path:
      upos_cluster_emb_layers.append(EmbeddingLayer(args2.cluster_dim, cluster_lexicon, fix_emb=False, embs=None))
      xpos_cluster_emb_layers.append(EmbeddingLayer(args2.cluster_dim, cluster_lexicon, fix_emb=False, embs=None))
    logging.info('size of cluster map: {0}'.format(len(cluster_lexicon)))
  else:
    cluster_lexicon = None
    upos_cluster_emb_layers = [None] * len(models_path)
    xpos_cluster_emb_layers = [None] * len(models_path)

  upos2id, id2upos = {}, {}
  with codecs.open(os.path.join(models_path[0], 'upos.dic'), 'r', encoding='utf-8') as fpi:
    for line in fpi:
      token, i = line.strip().split('\t')
      upos2id[token] = int(i)
      id2upos[int(i)] = token
  logging.info('number of upos: {0}'.format(len(upos2id)))

  xpos2id, id2xpos = {}, {}
  with codecs.open(os.path.join(models_path[0], 'xpos.dic'), 'r', encoding='utf-8') as fpi:
    for line in fpi:
      token, i = line.strip().split('\t')
      xpos2id[token] = int(i)
      id2xpos[int(i)] = token
  logging.info('number of xpos: {0}'.format(len(xpos2id)))
  ...
  for idx, path in enumerate(models_path):
    upos_models.append(Model(args2, upos_word_emb_layers[idx], upos_char_emb_layers[idx], upos_cluster_emb_layers[idx], len(upos2id), len(xpos2id), use_cuda))
    xpos_models.append(Model(args2, xpos_word_emb_layers[idx], xpos_char_emb_layers[idx], xpos_cluster_emb_layers[idx], len(upos2id), len(xpos2id), use_cuda))

    upos_models[-1].load_state_dict(torch.load(os.path.join(path, 'upos_model.pkl'), map_location=lambda storage, loc: storage))
    xpos_models[-1].load_state_dict(torch.load(os.path.join(path, 'xpos_model.pkl'), map_location=lambda storage, loc: storage))
    if use_cuda:
      upos_models[-1] = upos_models[-1].cuda()
      xpos_models[-1] = xpos_models[-1].cuda()
    
  test_x, test_y = read_corpus(args.input)
  label_to_index(test_y, upos2id, xpos2id, incremental=False)
  ...
  for model in upos_models:
    model.eval()
  for model in xpos_models:
    model.eval()

  ensemble_upos_model = EnsembleModel(upos_models)
  ensemble_xpos_model = EnsembleModel(xpos_models)

  ensemble_upos_model.eval()
  ensemble_xpos_model.eval()

  for x, c, e, cluster, y, lens, text in zip(test_x, test_c, test_e, test_cluster, test_y, test_lens, test_text):
    output_upos, loss = ensemble_upos_model.forward(x, c, e, cluster, y)
    output_xpos, loss = ensemble_xpos_model.forward(x, c, e, cluster, y)
    output_data = [[[u, x] for u, x in zip(upos, xpos)] for upos, xpos in zip(output_upos[0].data, output_xpos[1].data)]
    for bid in range(len(x)):
      for k, (word, tag) in enumerate(zip(text[bid], output_data[bid])):
        upos = id2upos[int(tag[0])]
        xpos = id2xpos[int(tag[1])]
        print('{0}\t{1}\t{1}\t{2}\t{3}\t_\t_\t_'.format(k + 1, word, upos, xpos), file=fpo)
      print(file=fpo)
  fpo.close()
